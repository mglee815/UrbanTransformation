...
for TARGET_CITY in TARGET_CITY_LIST:
    ...
    #========filter by city and separate into three data set======================
        ...
        for period, dataset in enumerate(data_lst):
            ...
            apriori_result.to_csv(f"../program_graph/city_topic_period/CTP_network_0906_featureKey/apriori_raw/C{TARGET_CITY}_T{topic+1}_P{period}_apriori.csv")
            ...
            try:
                ...
                plt.savefig(f"../program_graph/city_topic_period/CTP_network_0906_featureKey/C{TARGET_CITY}_T{topic + 1}_P{period}.png", format="PNG", dpi = 1000)
                print(f"Save ../program_graph/city_topic_period/CTP_network_0906_featureKey/C{TARGET_CITY}_T{topic + 1}_P{period} well")
                plt.clf()
        
    pd.DataFrame(data_len_lst).to_csv(f"../program_graph/city_topic_period/CTP_network_0906_featureKey/{TARGET_CITY}_article_num.csv")